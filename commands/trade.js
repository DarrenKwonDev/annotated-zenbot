...
module.exports = function (program, conf) {
  program
    ...
    // 현재 잔고 대비 몇 %나 매수, 매도 포지션을 잡을 수 있는지  
    ...
    // 조금 더 이득 보려고 매수1호가보다 좀 더 싸게 내놓고(markdown), 매도 1호가보다 좀 더 비싸게(markup) 내놓기 
    ...
    // 가격 오르면 매수 금지, 가격 내리면 매도 금지 (버티기 용)
    ...
    // 수익률이 5%에 도달하면 trailing stop 활성화
    // 그 시점부터 최고 수익률 대비 2% 아래에 stop 설정
    // 수익률이 10%까지 올라가면 stop도 8%로 올라감
    // 가격이 떨어져서 수익률이 8% 밑으로 가면 매도
    // trailing stop을 켜는 수익률 임계값
    ...
    // 결국 아래는 버티는 용도 
    // max_sell_loss_pct: 손실이 이 %를 넘으면 매도 안 함 (손절 거부)
    // max_buy_loss_pct: 손실이 이 %를 넘으면 추가 매수 안 함 (물타기 제한)
    ...
    // 이 슬리피지 이상이면 팔지 말아라. 슬리피지가 너무 크니까  
    ...
    // exact 옵션.
    // maker 매수호가 (즉시 체결 안되는 매수호가에 댄 주문) 가격이 내려가야만 조정하는데 exact를 켜면 가격이 올라가도 조정함
    // maker 매도호가는 가격이 올라야만 조정하는데 exact를 켜면 가격이 내려가도 조정함.
    ...
    // 너무 자주 trade하지 않도록 trade에 절대적인 시간 간격을 둠 
    .option('--interval_trade <minutes>', 'The interval trade time', Number, conf.interval_trade)
    // 손해 보고 있는 trade가 있으면 손실 직후 다시 매수하기 까지 시간을 둠 (격리 시간)
    ...
    .action(function (selector, cmd) {
      ...
      // lib/objectify-selector.js
      so.selector = objectifySelector(selector || conf.selector)

      // lib/engine.js
      var engine = engineFactory(s, conf)

      // lib/services/collection-service
      ...
      // --------------------------------------------------------------
      // 통계 계산. 지금까지 매매한 내역들 출력
      /*
        성과 지표로 아래 같은걸 쓰는듯
        s.stats = {
          profit: '12.5%',
          tmp_balance: '11250.00',
          buy_hold: '10500.00',
          error_rate: '15%',
          trade_per_day: '2.3'
        }
      */
      ...
      var backfiller = spawn(path.resolve(__dirname, '..', zenbot_cmd), command_args) // backfiller를 별도의 프로세스를 띄워서 작업하는 듯
