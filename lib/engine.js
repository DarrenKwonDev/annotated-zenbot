...
module.exports = function (s, conf) {
  ...
  function writeReport (is_progress, blink_off) {화면 출력 
    if ((so.mode === 'sim' || so.mode === 'train') && !so.verbose) {
      ...
      s.vol_since_last_blink = 0
      setTimeout(function () {
        writeReport(true, true)화면 출력 
      }, 200)
      
      setTimeout(function () {

        
        writeReport(true, false)화면 출력
  ...
  function onTrade(trade, is_preroll, cb) {
    ...
    if(trade.time > s.period.close_time ||
      ...
      s.strategy.onPeriod.call(s.ctx, s, function () {
        ...
        // preroll 기간도 아니고 수동도 아니라면 자동 매매 중인 것이다!
        ...
        // 완료된 period를 과거 데이터로 옮김  
        if (trade.time > s.period.close_time) {
          s.lookback.unshift(s.period)
          initBuffer(trade) // 다시 period를 생성
        }
